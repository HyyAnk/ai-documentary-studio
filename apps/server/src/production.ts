...
export function assessProduction(input: {
  ...
  if (uniquePromptRatio < 0.9) add("duplicate_prompts", "blocker", `${Math.round((1 - uniquePromptRatio) * 100)}% of prompts are exact duplicates.`, "Regenerate repeated shots with distinct visible action and framing.", 14, duplicatePromptScenes(scenes));
  if (structuredPromptRatio < 0.95) add("prompt_structure", "warning", `${Math.round(structuredPromptRatio * 100)}% of prompts contain the required production sections.`, "Complete CAMERA, ACTION, LIGHTING, ATMOSPHERE, and CONTINUITY for every generated shot.", 8);
  if (continuityCoverageRatio < 0.95) add("continuity_coverage", "blocker", `${Math.round(continuityCoverageRatio * 100)}% of shots are connected to a continuity bundle.`, "Assign every shot to a visual-bible continuity bundle.", 12);
  if (sourceCoverageRatio < 0.75) add("source_coverage", "warning", `${Math.round(sourceCoverageRatio * 100)}% of shots carry source IDs.`, "Link shots to research claims or evidence sources, including reconstructions.", 8);
