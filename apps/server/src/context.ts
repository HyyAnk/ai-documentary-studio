      ...
      : isQuiz && taskType === "GENERATE_TREATMENT"
        ...
        : isQuiz && taskType === "GENERATE_SCRIPT"
          ...
          : isQuiz && taskType === "GENERATE_VISUAL_BIBLE"
            ? `Return only a completed Markdown Quiz Visual Bible. Define a single child-friendly palette, typography, icon family, countdown treatment, question card, answer reveal, image-guess frame, and score moment. Include an explicit second-level section named exactly \`## Safe motion\` with labeled Allowed motion, Prohibited motion, and Reduced-motion fallback rules; the phrase \"safe motion\" must appear exactly in the document. Create exactly ${episode.quiz_config.question_count} continuity bundles formatted as \`## Continuity bundle CB-01 — Title\` through CB-${String(episode.quiz_config.question_count).padStart(2, "0")}. Every bundle must include an Anchor-frame prompt.`
