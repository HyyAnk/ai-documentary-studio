    ...
    const outputContract = isQuiz && taskType === "GENERATE_RESEARCH"
      ...
      : isQuiz && taskType === "GENERATE_TREATMENT"
        ...
        : isQuiz && taskType === "GENERATE_SCRIPT"
          ? `Return only one completed Markdown quiz narration script. Include exactly ${episode.quiz_config.question_count} question blocks, formatted as second-level headings \`## Question 1 — Title\` through Question ${episode.quiz_config.question_count}. Fold the welcome into Question 1 and the closing into the final question so the section count stays exact. For each block speak the question, choices or clue, an invitation to guess, the reveal, and one sourced-friendly explanation. Use concise child-friendly language for ages ${episode.quiz_config.age_band}. Add the exact hidden marker <!-- HUMOR_POLICY: v1 --> after the title. Use HTML comments only for restrained audio cues. Do not include JSON, planning notes, or visual directions.`
          : isQuiz && taskType === "GENERATE_VISUAL_BIBLE"
            ? `Return only a completed Markdown Quiz Visual Bible. Define a single child-friendly, vibrant, high-appeal visual direction with cheerful palette, typography, icon family, countdown treatment, question card, answer reveal, image-guess frame, and score moment. Visual style must cater directly to children (ages ${episode.quiz_config.age_band}): rich, colorful, highly engaging (such as 3D claymation, storybook 3D, or lively animated diorama), avoiding dark ink, flat dull 2D, or washed-out backgrounds. Include an explicit second-level section named exactly \`## Safe motion\` with labeled Allowed motion, Prohibited motion, and Reduced-motion fallback rules; the phrase \"safe motion\" must appear exactly in the document. Create exactly ${episode.quiz_config.question_count} continuity bundles formatted as \`## Continuity bundle CB-01 — Title\` through CB-${String(episode.quiz_config.question_count).padStart(2, "0")}. Every bundle must include an Anchor-frame prompt describing ONLY the clean hero subject illustration and environment with rich vibrant colors and clear depth. Never include question cards, answer choices, countdown timers, UI boxes, or text inside Anchor-frame prompts because the video composition engine renders all quiz UI dynamically on top of the clean artwork.`
