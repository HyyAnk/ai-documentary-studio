...
function thinkingBar(input: { clipStart: number; thinkingStart: number; revealStart: number }): string {
  const thinkingAt = Math.max(0, input.thinkingStart - input.clipStart);
  const duration = Math.max(.05, input.revealStart - input.thinkingStart);
  ...
  const style = `style="--thinking-at:${thinkingAt.toFixed(3)}s;--thinking-duration:${duration.toFixed(3)}s;--timer-duration:${duration.toFixed(3)}s;--cd-query-dur:${queryDuration.toFixed(3)}s;--cd5-at:${cd5.toFixed(3)}s;--cd4-at:${cd4.toFixed(3)}s;--cd3-at:${cd3.toFixed(3)}s;--cd2-at:${cd2.toFixed(3)}s;--cd1-at:${cd1.toFixed(3)}s"`;
...
.thinking-bar { position: relative; z-index: 5; display: flex; align-items: center; width: 1380px; min-height: 80px; margin-top: 0; padding: 12px 24px; border: 0; border-radius: 999px; background: transparent; box-shadow: none; opacity: 0; animation: phase-hold var(--thinking-duration) steps(1,end) calc(var(--clip-start) + var(--thinking-at)) both, timer-exit-fade .28s cubic-bezier(.22,.8,.3,1) calc(var(--clip-start) + var(--thinking-at) + var(--thinking-duration) - .28s) both; }
.thinking-track { position: relative; z-index: 5; width: 100%; height: 52px; overflow: visible; border: 5px solid rgba(255,255,255,.9); border-radius: 999px; background: rgba(16,35,72,.35); box-shadow: inset 0 4px 6px rgba(0,0,0,.22), 0 7px 0 var(--depth-edge), 0 0 24px rgba(255,255,255,.22); animation: timer-urgency-glow var(--thinking-duration) ease-in calc(var(--clip-start) + var(--thinking-at)) both; }
...
.val-query { opacity: 1; animation: query-hold var(--cd-query-dur) steps(1,end) calc(var(--clip-start) + var(--thinking-at)) both; }
.val-5 { animation: number-countdown-tick 1s cubic-bezier(.18,1.42,.34,1) calc(var(--clip-start) + var(--thinking-at) + var(--cd5-at)) both; }
.val-4 { animation: number-countdown-tick 1s cubic-bezier(.18,1.42,.34,1) calc(var(--clip-start) + var(--thinking-at) + var(--cd4-at)) both; }
.val-3 { animation: number-countdown-tick 1s cubic-bezier(.18,1.42,.34,1) calc(var(--clip-start) + var(--thinking-at) + var(--cd3-at)) both; }
.val-2 { animation: number-countdown-tick 1s cubic-bezier(.18,1.42,.34,1) calc(var(--clip-start) + var(--thinking-at) + var(--cd2-at)) both; }
.val-1 { animation: number-countdown-final 1s cubic-bezier(.18,1.42,.34,1) calc(var(--clip-start) + var(--thinking-at) + var(--cd1-at)) both; }
.timer-sparkles { position: absolute; inset: -18px -12px; pointer-events: none; }
.timer-sparkles i { position: absolute; color: var(--accent); font-size: 22px; font-style: normal; animation: timer-sparkle var(--thinking-duration) ease-in-out calc(var(--clip-start) + var(--ambient-phase)) 1 both; }
.timer-sparkles i:nth-child(1) { right: 7%; top: -17px; }.timer-sparkles i:nth-child(2) { right: 1%; bottom: -15px; color: #FFD34D; font-size: 18px; animation-delay: calc(var(--clip-start) + var(--thinking-at) + .55s); }.timer-sparkles i:nth-child(3) { left: 4%; top: -14px; color: #fff; animation-delay: calc(var(--clip-start) + var(--thinking-at) + 1.05s); }
