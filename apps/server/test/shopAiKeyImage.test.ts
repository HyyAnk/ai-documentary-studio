...
const originalEnv = { key: process.env.SHOPAIKEY_API_KEY, base: process.env.SHOPAIKEY_BASE_URL, model: process.env.SHOPAIKEY_IMAGE_MODEL, size: process.env.SHOPAIKEY_IMAGE_SIZE, quality: process.env.SHOPAIKEY_IMAGE_QUALITY };

afterEach(async () => {
  globalThis.fetch = originalFetch;
  for (const [name, value] of Object.entries({ SHOPAIKEY_API_KEY: originalEnv.key, SHOPAIKEY_BASE_URL: originalEnv.base, SHOPAIKEY_IMAGE_MODEL: originalEnv.model, SHOPAIKEY_IMAGE_SIZE: originalEnv.size, SHOPAIKEY_IMAGE_QUALITY: originalEnv.quality })) {
