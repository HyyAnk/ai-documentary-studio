...
import type { AppConfig, Channel, CodexSettingsResponse, StorageInfo, Task } from "@studio/shared";
import { api } from "./api";
...
export function App() {
  ...
  return <div className="app-shell"><Sidebar page={page} setPage={navigate} activeTaskCount={activeTasks.length} /><main className="main-column"><Topbar channel={selectedChannel} codexStatus={codexStatus} git={git} codex={codex} theme={theme} onThemeToggle={() => setTheme((current) => current === "dark" ? "light" : "dark")} onModelChange={async (model) => { try { await saveCodex({ model }); showGood(model ? `Model: ${model}` : "Using Codex default model"); } catch (error) { showError(error); } }} onReconnect={async () => { try { const result = await api.reconnectCodex(); setCodexStatus(result.status); if (result.status === "connected") showGood("Codex connected"); else showError(new Error(result.message || "Codex unavailable")); } catch (error) { showError(error); } }} onShutdown={() => void stopDashboard()} /><TaskActivityBar tasks={activeTasks} realtimeStatus={realtimeStatus} now={taskClock} onOpenTasks={() => navigate("tasks")} />{loading ? <LoadingState /> : page === "dashboard" ? <DashboardView channels={channels} tasks={tasks} activeTasks={activeTasks} now={taskClock} onCreate={() => requestCreateChannel("quiz")} openChannel={openChannel} onDelete={requestDeleteChannel} openTaskList={() => navigate("tasks")} openChannelsList={() => navigate("channels")} /> : null}{!loading && page === "channels" ? <ChannelsView selectedChannel={selectedChannel} selectedEpisodeId={selectedEpisodeId} channels={channels} tasks={tasks} onTaskSubmitted={upsertTask} openChannel={openChannel} onCreate={requestCreateChannel} onRefresh={refresh} onNotice={setNotice} onDelete={requestDeleteChannel} openEpisode={openEpisode} maxDuration={appConfig?.video_generation.max_scene_duration_seconds ?? 8} narrationWordsPerSecond={appConfig?.video_generation.narration_words_per_second ?? 2.3} imageGenerationEnabled={appConfig?.image_generation?.enabled ?? true} imagesPerBundle={appConfig?.image_generation?.images_per_bundle ?? 1} /> : null}{!loading && page === "tasks" ? <TasksView tasks={tasks} now={taskClock} onRefresh={refresh} onNotice={setNotice} /> : null}{!loading && page === "settings" ? <SettingsView channels={channels} appConfig={appConfig} codex={codex} codexStatus={codexStatus} git={git} storage={storage} onStorageSaved={applyStorage} onCodexSaved={setCodex} onAudioSaved={(audio) => setAppConfig((current) => current ? { ...current, audio_generation: audio } : current)} onVideoSaved={(video) => setAppConfig((current) => current ? { ...current, video_generation: video } : current)} onImageSaved={(image) => setAppConfig((current) => current ? { ...current, image_generation: image } : current)} onChannelUpdated={(channel) => setChannels((current) => current.map((item) => item.channel_id === channel.channel_id ? channel : item))} onNotice={setNotice} /> : null}<footer className="app-credit"><span className="app-credit-full">Develop - Design - Deliver by HyyAnk | Dư Ngọc Minh Hoàng</span><span className="app-credit-mobile">HyyAnk | Dư Ngọc Minh Hoàng</span></footer></main>{storage && !storage.configured ? <StorageSetupModal storage={storage} onSaved={async (next) => { await applyStorage(next); showGood("Content storage is ready"); }} onError={showError} /> : null}{showCreate ? <CreateChannelModal initialGroupId={showCreate} onClose={() => setShowCreate(null)} onCreated={async (channelId, message, task) => { if (task) upsertTask(task); setShowCreate(null); await refresh(); openChannel(channelId); setNotice({ tone: "good", message }); }} onError={showError} /> : null}{deleteTarget ? <DeleteChannelModal channel={deleteTarget} onClose={() => setDeleteTarget(null)} onDeleted={handleChannelDeleted} onError={showError} /> : null}{notice ? <NoticeBanner notice={notice} onClose={() => setNotice(null)} /> : null}</div>;
}
