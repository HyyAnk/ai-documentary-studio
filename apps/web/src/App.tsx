  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-column">
        ...
        {!loading && page === "channels" ? <ChannelsView selectedChannel={selectedChannel} channels={channels} tasks={tasks} openChannel={openChannel} onCreate={() => setShowCreate(true)} onRefresh={refresh} onNotice={setNotice} openEpisode={openEpisode} /> : null}
...
function ChannelsView({ selectedChannel, channels, tasks, openChannel, onCreate, onRefresh, onNotice, openEpisode }: { selectedChannel: Channel | null; channels: Channel[]; tasks: Task[]; openChannel: (id: string) => void; onCreate: () => void; onRefresh: () => Promise<void>; onNotice: (notice: Notice) => void; openEpisode: (channelId: string, episodeId: string) => void }) {
  if (selectedChannel) return <ChannelDetail channel={selectedChannel} channels={channels} tasks={tasks} onBack={() => openChannel("")} onRefresh={onRefresh} onNotice={onNotice} openEpisode={openEpisode} />;
...
function ChannelDetail({ channel, channels, tasks, onBack, onRefresh, onNotice, openEpisode }: { channel: Channel; channels: Channel[]; tasks: Task[]; onBack: () => void; onRefresh: () => Promise<void>; onNotice: (notice: Notice) => void; openEpisode: (channelId: string, episodeId: string) => void }) {
  ...
  const topicTask = tasks.filter((task) => task.channel_id === channel.channel_id && task.task_type === "SUGGEST_TOPICS").sort((a, b) => b.created_at.localeCompare(a.created_at))[0] ?? null;
  const topicTaskActive = topicTask ? ["QUEUED", "RUNNING", "WAITING_APPROVAL"].includes(topicTask.status) : false;
  ...
  useEffect(() => {
    if (topicTask?.status !== "COMPLETED") return;
    void load().catch((error: Error) => onNotice({ tone: "bad", message: error.message }));
  }, [load, onNotice, topicTask?.task_id, topicTask?.status]);
  const suggest = async () => { if (topicTaskActive) return; setBusy("topics"); try { await api.suggestTopics(channel.channel_id); onNotice({ tone: "good", message: "Generating 5 lightweight topic ideas…" }); } catch (error) { onNotice({ tone: "bad", message: (error as Error).message }); } finally { setBusy(null); } };
...
function CreateChannelModal({ onClose, onCreated, onError }: { onClose: () => void; onCreated: (channelId: string, message: string) => Promise<void>; onError: (error: unknown) => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await onCreated(result.channel.channel_id, message);
    } catch (error) {
      onError(error);
    } finally {
      setBusy(false);
...
function TopicProgress({ task, now }: { task: Task; now: number }) {
  const active = ["QUEUED", "RUNNING", "WAITING_APPROVAL"].includes(task.status);
  const completed = task.status === "COMPLETED";
  const failed = task.status === "FAILED";
  const label = completed ? "5 topics ready" : failed ? "Topic generation failed" : task.status === "WAITING_APPROVAL" ? "Waiting for approval" : "Generating 5 topics";
  ...
  return <div className={`topic-progress ${task.status.toLowerCase()}`} role="status"><div className="topic-progress-head"><div className="topic-progress-title"><span className="eyebrow">Topic generation</span><strong>{label}</strong></div><span className="topic-progress-time">{formatElapsed(start, end)}</span></div><div className="topic-progress-track" role="progressbar" aria-label="Topic generation progress" aria-valuetext={completed ? "Complete" : failed ? "Failed" : progressMessage}><span className="topic-progress-fill" /></div><p className="topic-progress-copy">{progressMessage}{active ? " — this panel will update automatically" : ""}</p></div>;
}
