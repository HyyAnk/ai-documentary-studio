...
function SessionCleanupToggle({ engine, enabled, disabled, onChange }: { engine: "Codex" | "Antigravity"; enabled: boolean; disabled: boolean; onChange: (enabled: boolean) => void }) {
  return <label className="session-cleanup-toggle"><span>Session cleanup</span><span className="session-cleanup-switch"><input type="checkbox" role="switch" aria-label={`Enable ${engine} session cleanup`} checked={enabled} disabled={disabled} onChange={(event) => onChange(event.target.checked)} /><span className="session-cleanup-track" aria-hidden="true"><span>Off</span><span>On</span><i /></span></span></label>;
}
...
export function SettingsView({ channels, appConfig, codex, codexStatus, antigravity, antigravityStatus, git, storage, onStorageSaved, onCodexSaved, onAntigravitySaved, onAudioSaved, onVideoSaved, onImageSaved, onChannelUpdated, onNotice }: SettingsViewProps) {
  ...
  const cleanupAntigravity = async () => {
    ...
    try { const result = await api.cleanupAntigravity(); onNotice({ tone: "good", message: result.removed ? `${result.removed} old Antigravity session${result.removed === 1 ? "" : "s"} cleaned up` : "No old Antigravity sessions needed cleanup" }); }
    catch (error) { onNotice({ tone: "bad", message: error instanceof Error ? error.message : "Could not clean up Antigravity sessions" }); }
    finally { setCleaningAgyThreads(false); }
  };
  ...
  return <section className="page-wrap"><PageTitle eyebrow="Workspace" title="Settings" /><div className="settings-grid">
    ...
    <section className="panel image-settings-panel"><div className="panel-heading"><div><p className="eyebrow">Image provider</p><h2>gpti2.store</h2></div><FileText size={22} /></div><StatusLine label="Provider" value="gpti2.store (API)" /><StatusLine label="API Key status" value={hasImageApiKey ? "Configured" : "Not configured"} />{imageBalanceInfo ? <StatusLine label="Available balance" value={`${imageBalanceInfo.balance_vnd.toLocaleString("en-US")} VND${imageBalanceInfo.rpm ? ` (RPM: ${imageBalanceInfo.rpm})` : ""}`} /> : null}<form className="codex-form" onSubmit={(event) => void saveImage(event)}><label className="toggle-field"><input type="checkbox" checked={imageEnabled} onChange={(event) => setImageEnabled(event.target.checked)} />Enable continuity anchor images</label><label>gpti2.store API key<div style={{ display: "flex", gap: "6px", alignItems: "center", width: "100%" }}><input type={showImageKey ? "text" : "password"} value={imageApiKey} onChange={(event) => setImageApiKey(event.target.value)} placeholder="Paste gpti2.store API key (sk-...)" autoComplete="off" style={{ flex: 1 }} /><button type="button" className="quiet-button compact" title={showImageKey ? "Hide key" : "Show key"} onClick={() => setShowImageKey(!showImageKey)} style={{ height: "35px", padding: "0 10px" }}>{showImageKey ? <EyeSlash size={16} /> : <Eye size={16} />}</button>{hasImageApiKey || imageApiKey ? <button type="button" className="icon-button danger compact" title="Remove this API Key" disabled={savingImage} onClick={() => void clearImageKey()} style={{ height: "35px", width: "35px", minWidth: "35px", borderRadius: "6px" }}><Trash size={16} /></button> : null}</div><small className="field-help">{hasImageApiKey ? "Key stored securely in .documentary-studio/ (gitignored). You can edit directly to replace or click the trash icon to remove." : "Get your API key from the Account tab at https://gpti2.store. Stored securely in .documentary-studio/ (gitignored)."}</small></label><label>Default Model<select value={imageModel} onChange={(event) => setImageModel(event.target.value)}><option value="gpt-image-2">gpt-image-2 (50 VND / image - Economy)</option><option value="nano-banana-2">nano-banana-2 (100 VND / image - 2K)</option></select></label><label>Parallel Generation Threads<select value={maxConcurrentImageTasks} onChange={(event) => setMaxConcurrentImageTasks(Number(event.target.value))}><option value="1">1 worker</option><option value="2">2 workers</option><option value="3">3 workers (Recommended)</option><option value="4">4 workers</option></select></label><label>Images per bundle<select value={imagesPerBundle} disabled={!imageEnabled} onChange={(event) => setImagesPerBundle(Number(event.target.value))}><option value="1">1 anchor</option><option value="2">2 anchors</option></select></label><small className="field-help">Configured with low quality to optimize cost. Idempotency protection and async queue support are active.</small><div style={{ display: "flex", gap: "8px", alignItems: "center" }}><button className="primary-button" disabled={savingImage}>{savingImage ? <CircleNotch className="spin" size={16} /> : <FloppyDisk size={16} />}Save images</button>{hasImageApiKey ? <button type="button" className="quiet-button" disabled={checkingImageBalance} onClick={() => void checkImageBalance()}>{checkingImageBalance ? <CircleNotch className="spin" size={15} /> : null}{checkingImageBalance ? "Checking…" : "Check balance & Key"}</button> : null}</div></form></section>
