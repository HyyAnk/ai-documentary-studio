...
export function SettingsView({ channels, appConfig, codex, codexStatus, git, storage, onStorageSaved, onCodexSaved, onAudioSaved, onVideoSaved, onChannelUpdated, onNotice }: SettingsViewProps) {
  ...
  const [targetSceneDuration, setTargetSceneDuration] = useState(appConfig?.video_generation.target_scene_duration_seconds ?? 8);
  const [minSceneDuration, setMinSceneDuration] = useState(appConfig?.video_generation.min_scene_duration_seconds ?? 4);
  ...
  useEffect(() => {
    ...
    if (audio) { setAudioUrl(audio.service_url); setExaggeration(audio.exaggeration); setCfgWeight(audio.cfg_weight); setMergeGapMs(audio.merge_gap_ms); }
    if (video) { setTargetSceneDuration(video.target_scene_duration_seconds); setMinSceneDuration(video.min_scene_duration_seconds); }
  }, [appConfig]);
  ...
  const saveAudio = async (event: FormEvent) => {
    ...
    try { const next = await api.saveAudioSettings({ provider: "chatterbox", service_url: audioUrl, exaggeration, cfg_weight: cfgWeight, max_concurrent_tasks: appConfig?.audio_generation.max_concurrent_tasks ?? 2, merge_gap_ms: mergeGapMs }); await onAudioSaved(next.audio_generation); onNotice({ tone: "good", message: "Audio settings saved locally" }); }
  ...
  const saveVideo = async (event: FormEvent) => {
    ...
    try { const next = await api.saveVideoSettings({ target_scene_duration_seconds: targetSceneDuration, min_scene_duration_seconds: minSceneDuration }); await onVideoSaved(next.video_generation); onNotice({ tone: "good", message: "Video packing settings saved locally" }); }
  ...
  return <section className="page-wrap"><PageTitle eyebrow="Workspace" title="Settings" /><div className="settings-grid">
    ...
    <section className="panel video-settings-panel"><div className="panel-heading"><div><p className="eyebrow">Video generation</p><h2>Scene packing</h2></div><VideoCamera size={22} /></div><StatusLine label="Hard maximum" value={`${maxDuration}s`} /><form className="codex-form" onSubmit={(event) => void saveVideo(event)}><label>Target duration<input type="number" min="1" max={maxDuration} step="0.5" value={targetSceneDuration} onChange={(event) => setTargetSceneDuration(Number(event.target.value))} /></label><label>Soft minimum<input type="number" min="1" max={maxDuration} step="0.5" value={minSceneDuration} onChange={(event) => setMinSceneDuration(Number(event.target.value))} /></label><button className="primary-button" disabled={savingVideo}>{savingVideo ? <CircleNotch className="spin" size={16} /> : <FloppyDisk size={16} />}Save video</button></form></section>
    <section className="panel audio-settings-panel"><div className="panel-heading"><div><p className="eyebrow">Local speech</p><h2>Audio</h2></div><SpeakerHigh size={22} /></div><StatusLine label="Provider" value="Chatterbox" /><form className="codex-form" onSubmit={(event) => void saveAudio(event)}><label>Service URL<input value={audioUrl} onChange={(event) => setAudioUrl(event.target.value)} placeholder="http://127.0.0.1:8890" /></label><label>Exaggeration<input type="number" min="0" max="1" step="0.05" value={exaggeration} onChange={(event) => setExaggeration(Number(event.target.value))} /></label><label>CFG weight<input type="number" min="0" max="1" step="0.05" value={cfgWeight} onChange={(event) => setCfgWeight(Number(event.target.value))} /></label><label>Merge gap (ms)<input type="number" min="0" max="10000" step="50" value={mergeGapMs} onChange={(event) => setMergeGapMs(Number(event.target.value))} /></label><button className="primary-button" disabled={savingAudio}>{savingAudio ? <CircleNotch className="spin" size={16} /> : <FloppyDisk size={16} />}Save audio</button></form></section>
