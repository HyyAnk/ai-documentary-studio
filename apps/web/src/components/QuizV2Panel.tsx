...
export function QuizV2Panel({ state, busy, onRun }: { state: QuizV2State | null; busy: string | null; onRun: (stage: QuizStage) => void }) {
  ...
  return <section className="panel quiz-v2-panel" aria-labelledby="quiz-v2-title">
    <div className="panel-heading">
      ...
      {nextStage && nextStage.key !== "render" ? <button className="primary-button compact" disabled={Boolean(pendingStage) || !canRun(nextStage.key, state)} onClick={() => onRun(nextStage.key)}>{pendingStage === nextStage.key ? <CircleNotch className="spin" size={15} /> : <Play size={15} />}{pendingStage === nextStage.key ? "Working" : actionLabel(nextStage.key, state)}</button> : null}
    </div>
    <ol className="quiz-v2-rail" aria-label="Quiz Engine V2 production stages">
      {stages.map((stage) => {
        ...
        const available = canRun(stage.key, state);
...
function canRun(stage: QuizStage, state: QuizV2State): boolean {
  if (stage === "research") return false;
  if (stage === "questions") return true;
