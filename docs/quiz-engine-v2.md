...
Quiz Engine V2 adds a versioned, local-first production chain to the existing Quiz channel workflow:

    scene facts → QuizV2 → DirectorPlan → AssetPlan + VoicePlan → QuizTimeline → pre-render QA → HyperFrames → post-render QA
